...
ejs.renderFile(filename, data, options, function(err, str)***REMOVED***
...
## Options
  ...
  - `_with`                 Whether or not to use `with() ***REMOVED******REMOVED***` constructs. If `false`
...
<ul>
  <% users.forEach(function(user)***REMOVED*** %>
    <%- include('user/show', ***REMOVED***user: user***REMOVED***) %>
  <% ***REMOVED***); %>
</ul>
```
...
// Just one template
ejs.render('<?= users.join(" | "); ?>', ***REMOVED***users: users***REMOVED***, ***REMOVED***delimiter: '?'***REMOVED***);
// => 'geddy | neil | alex'

// Or globally
ejs.delimiter = '$';
ejs.render('<$= users.join(" | "); $>', ***REMOVED***users: users***REMOVED***);
// => 'geddy | neil | alex'
```
...
```javascript
let ejs = require('ejs');
let myFileLoad = function (filePath) ***REMOVED***
...
<script>
  let people = ['geddy', 'neil', 'alex'],
      html = ejs.render('<%= people.join(", "); %>', ***REMOVED***people: people***REMOVED***);
  // With jQuery:
  $('#output').html(html);
  // Vanilla JS:
...
2. For the same reason, `include`s do not work unless you use an `include callback`. Here is an example:
  ```javascript
  let str = "Hello <%= include('file', ***REMOVED***person: 'John'***REMOVED***); %>",
      fn = ejs.compile(str, ***REMOVED***client: true***REMOVED***);

  fn(data, null, function(path, d)***REMOVED*** // include callback
    // path -> 'file'
    // d -> ***REMOVED***person: 'John'***REMOVED***
    // Put your code here
